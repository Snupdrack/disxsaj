...
async function main() {
  ...
  // Create Sabores (only if none exist)
  const existingSabores = await prisma.sabor.count()
  if (existingSabores === 0) {
    const sabores = [
  ...
  const existingExtrasClasico = await prisma.extraClasico.count()
  if (existingExtrasClasico === 0) {
    const extrasClasico = [
  ...
  const existingExtrasDiablito = await prisma.extraDiablito.count()
  if (existingExtrasDiablito === 0) {
  ...
  const existingExtrasGloria = await prisma.extraGloria.count()
  if (existingExtrasGloria === 0) {
    const extrasGloria = [
  ...
  const existingBases = await prisma.baseDiablito.count()
  if (existingBases === 0) {
    const basesDiablito = [
  ...
  // Create Botanas
  const existingBotanas = await prisma.botana.count()
  if (existingBotanas === 0) {
    const botanas = [
